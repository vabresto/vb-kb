---
org: The Table
alias:
website: https://thetable.la
hq-location:
stages: []
check-size:
thesis: Founder community centered on partnership-driven dinner gatherings.
focus-sectors:
- founder community
- networking
portfolio-examples: []
created-at: 2026-02-16
updated-at: 2026-02-16
relationship-status: research
known-people:
- person: "[Matthew Brannon](../person/matthew-brannon.md)"
  relationship: current
  relationship-details: Current founder relationship.
  relationship-start-date: 2023-09
  relationship-end-date: null
  first-noted-at: 2026-02-16
  last-verified-at: 2026-02-16
intro-paths: []
last-updated-from-source: 2026-02-16
---

# The Table

## Snapshot

- Why this org matters: Current organization listed for Matthew Brannon.
- What they do: Founder-focused community and events.
- Best way to approach: Start with partnership intent and clear founder value.

## Bio

The Table describes itself as an invite-only dinner community for seven-figure founders focused on high-trust partnerships.[^thetable-source]

## Notes

- [2026-02-16] Organization page created to support linked employment references.

## Changelog

- [2026-02-16]: Created page with sourced bio summary

[^thetable-source]: Source: [The Table](https://thetable.la/). Verified/accessed on 2026-02-16.
