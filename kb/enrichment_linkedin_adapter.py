...
LINKEDIN_FETCH_COMMAND_ENV_VAR = "KB_ENRICHMENT_LINKEDIN_FETCH_COMMAND"
...
class LinkedInExtractionError(SourceAdapterError):
    def __init__(self, *, reason: str, details: str | None = None) -> None:
        super().__init__(
            source=SupportedSource.linkedin,
            message=f"linkedin extraction failed: {reason}",
            details=details,
        )


class LinkedInFetchCommandResult(KBBaseModel):
    returncode: int
    stdout: str = ""
    stderr: str = ""


LinkedInFetchRunner = Callable[[list[str], Mapping[str, str], Path], LinkedInFetchCommandResult]
...
class LinkedInSourceAdapter(SourceAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        fetch_runner: LinkedInFetchRunner | None = None,
    ...
    def fetch(self, request: FetchRequest) -> FetchResult:
        ...
        if fetch_command is None:
            raise LinkedInExtractionError(
                ...
                details=(
                    f"set '{LINKEDIN_FETCH_COMMAND_ENV_VAR}', set source fetch_command in EnrichmentConfig, "
        ...
        if _is_unsupported_payload(payload):
            raise LinkedInExtractionError(
                reason="unsupported linkedin extraction flow",
...
def _default_fetch_runner(argv: list[str], env: Mapping[str, str], cwd: Path) -> LinkedInFetchCommandResult:
    ...
    return LinkedInFetchCommandResult(
...
def _facts_from_list(facts_payload: list[Any], *, fetch_result: FetchResult) -> list[NormalizedFact]:
    ...
    for index, raw_fact in enumerate(facts_payload):
        ...
        normalized_metadata.setdefault("adapter", SupportedSource.linkedin.value)
    ...
    *,
    ...
    for source_field, target_attribute in field_map.items():
        ...
        if value is None:
            continue
        metadata = {
            "adapter": SupportedSource.linkedin.value,
...
def _resolve_source_url(payload: dict[str, Any], *, entity_slug: str) -> str:
    ...
    if isinstance(value, str) and value.strip():
        return value.strip()
    return f"https://www.linkedin.com/in/{entity_slug}/"
